...
func New(cfg map[string]string) (*Store, error) {
	...
	s := &Store{client: client, clientID: clientID, configAppID: cfg["app_id"]}
...
func (s *Store) upload(ctx context.Context, req *store.UploadRequest) error {
	...
	// Upload APK
	if err := s.uploadAPK(appID, req.FilePath); err != nil {
...
func (s *Store) fetchAppID(packageName string) (string, error) {
	...
	if resp.Ret.Code != 0 {
		return "", fmt.Errorf("%s", resp.Ret.Message)
	}
...
func (s *Store) uploadAPK(appID, apkPath string) error {
...
func (s *Store) submitApp(appID string) *retInfo {
	var resp struct {
		Ret retInfo `json:"ret"`
	}
	s.client.R().
